...
function getRandomReward() {
  ...
  if (rand < 0.001) return 1000;
  if (rand < 0.011) return 100;
  if (rand < 0.044) return 27;
  if (rand < 0.21) return 9;
  if (rand < 0.41) return 6;
  if (rand < 0.71) return 4;
  if (rand < 0.85) return 2;
  if (rand < 0.95) return 1.6;
  return 1.2;
}
...
document.getElementById('dropBall').addEventListener('click', () => {
  ...
  if (!isNaN(enteredBet)) {
    ...
    betInput.value = currentBet; // input ichidagi qiymatni yangilash
  } else {
    currentBet = MIN_BET;
